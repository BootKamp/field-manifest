# The Field Manifest — Netlify Deployment Guide

Editorial 4WD trip planner for BootKamp. Packing list, payload tracker, journal, and live fuel route planning, packaged as a Next.js app that deploys to Netlify and installs to phones as a PWA.
...
├── netlify/
│   └── functions/
│       └── plan-route.js    ← serverless function that proxies Anthropic API
├── public/
│   ├── manifest.webmanifest ← PWA manifest
│   └── icons/               ← app icons (FM monogram)
├── styles/
...
Plan to spend about 30 minutes the first time. Once it's live, you push code changes and they're deployed in about 90 seconds.

### Prerequisites

1. **A GitHub account** — for hosting the source code. Free.
2. **A Netlify account** — for deploying the app. Free tier is plenty.
...
3. Select GitHub, then find and click your `field-manifest` repo.
4. **Crucial:** before clicking Deploy, go to **Environment** and add a new variable:
   - Key: `ANTHROPIC_API_KEY`
   - Value: paste your real key (starts with `sk-ant-...`)
5. Click **Deploy**.

Netlify builds and deploys in about 2–3 minutes. When it finishes you'll see a URL like `field-manifest-abc123.netlify.app`. That URL already works — the app is live.

### Step 3: Point your custom domain at it

1. In Netlify, go to your site → **Domain management**.
2. Click **Add domain** and enter `app.bootkamp.co` (or whatever subdomain you prefer).
3. Netlify shows you the DNS records to add. They'll be a CNAME pointing to `field-manifest-abc123.netlify.app`.
...
5. Wait 5–30 minutes for DNS to propagate. Netlify detects when it's ready and auto-issues a TLS certificate.
...
Once the initial setup is done, deploying is trivial:

1. Make your code changes locally (or edit directly in GitHub).
2. Push to GitHub.
3. Netlify detects the push and automatically rebuilds and redeploys.
4. Live in 60–90 seconds. No manual step needed.
...
Open http://localhost:3000 in your browser. Edits to any file hot-reload. When you push to GitHub, Netlify rebuilds the live site automatically.
...
- **Netlify hosting:** $0/month on the free tier. Sufficient unless you get serious traffic (>100 GB/month bandwidth). Pro is $19/month if you need more.
...
You forgot to add `ANTHROPIC_API_KEY` to Netlify's environment variables. Go to Netlify → your site → Site settings → Build & deploy → Environment → add the variable, then redeploy from the Deployments tab.

**Fuel search times out after 10 seconds**
Netlify's serverless functions have a timeout. For longer-running web searches (which can take 15–30s), you need to upgrade or increase the timeout. Check Netlify's docs on function timeouts. Pro tier gives you more headroom.

**PWA install prompt doesn't appear on Android**
Chrome requires the site to be served over HTTPS (Netlify does this automatically) and the manifest + service worker to be valid. If it doesn't appear, open Chrome DevTools on a laptop while viewing the site → Application tab → Manifest. Errors will be listed there.
...
The fuel planner makes a single fetch to `/.netlify/functions/plan-route`, which on the server side authenticates with Anthropic and forwards the prompt + web search tool. The response (a JSON object with stops, coordinates, prices, advice) is rendered into the SVG route map plus the stop list.
